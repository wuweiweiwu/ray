...
class ComponentExample extends Component {
  ...
  render() {
    const { recipe, codepenSlug, experimental } = this.props;
    ...
    const lightUIclassnames = classnames({
      ...
      'bx--global-light-ui': recipe === 'tabs'
    });
    ...
    let componentName =
      recipe
        .replace(/-/g, ' ')
        .charAt(0)
        .toUpperCase() + recipe.replace(/-/g, ' ').substring(1);
    ...
    const liveBackgroundClasses = classnames(
      ...
      `component-example__live--${recipe}`
    );
    ...
    return (
      <div className={lightUIclassnames}>
        ...
        <CodeExample htmlFile={currentHTMLfile} expandedDefault={true} />
      </div>
    );
  }
}
